(function(){
  ...
  appRun.$inject = ['$ionicPlatform', '$rootScope', '$state', '$window', '$ionicLoading', 'store', 'jwtHelper', 'config', 'globalData', 'stateConfig', 'userData', 'workout', 'modalUtils', 'exerciser', '$cordovaNetwork', '$cordovaAppRate'];

  function appRun($ionicPlatform, $rootScope, $state, $window, $ionicLoading, store, jwtHelper, config, globalData, stateConfig, userData, workout, modalUtils, exerciser, $cordovaNetwork, $cordovaAppRate){
...
//        analytics.startTrackerWithId("UA-60155665-4");
//        analytics.trackView("adds");
      } else {
        console.log("Google Analytics Unavailable");
      }
      
      //Adjust
      //Set EnvironmentSandbox to EnvironmentProduct before release of app
      var adjustConfig = new AdjustConfig("avdjq10fygw0", AdjustConfig.EnvironmentSandbox);
...
/*      if(window.plugins && window.AdMob){
        ...
        AdMob.createBanner( {
        adId: "ca-app-pub-9957314535048944/4336016913",
...
//    });
          ...
          if ($rootScope.isInTransition){
            ...
            modalUtils.launch('youSure', function(){
              ...
              if (toState.name == 'nav.customizeWorkout' || toState.name == 'nav.exercise'){
                ...
                $state.go(toState.name).then(function(){$ionicLoading.hide()});
              }
            });
          };
        }
    ...
    $window.onbeforeunload = function(event){
      ...
      var states = ['nav.customizeWorkout', 'nav.exercise'];
