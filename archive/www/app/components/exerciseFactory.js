(function(){
  ...
  function exerciser($interval, $state, $cordovaNetwork, ngAudio, userData, workout, $ionicLoading, $timeout){
    ...
    sound.begin = ngAudio.load("content/audio/5_sec_countdown_begin.mp3");
    sound.rest = ngAudio.load("content/audio/5_sec_countdown_rest.mp3");
    var paused = true;
    var onePlay = true;
    var currentTime;
    ...
    function stopper(){
      var vs = this;
      var timespan = 0;
      if(onePlay && vs.user.settings.sound){
        sound.start.play();
        onePlay=false;
      }
      if(isSet){
        ...
        if(vs.currentTime > 1){
          vs.currentTime -= 1;
          if(vs.currentTime == 5 && vs.user.settings.sound){
            sound.rest.play();
          }
    ...
    function startPauseClick(vs, shush){
      ...
      if(vs.user.settings.sound && shush && paused && !onePlay){
        if(isSet){
          sound.rest.pause();
        }else{
          sound.begin.pause();
        ...
        paused = !paused;
      }else if(vs.user.settings.sound && shush && !paused && !onePlay){
      ...
      if(angular.isDefined(stop)){
        if(vs.user.settings.sound && shush){
          sound.pause.play();
        }
        pause(vs);
        ...
        if(!onePlay && vs.user.settings.sound && shush){
          sound.resume.play();
        }
        start(vs);
        if(isSet){
